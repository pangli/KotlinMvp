...
object CacheDataUtil {
    ...
    fun getFormatSize(size: Double, newScale: Int? = 2): String {
        ...
        if (megaByte < 1) {
            ...
            return result1.setScale(newScale ?: 2, BigDecimal.ROUND_HALF_UP).toPlainString() + "KB"
        }
        ...
        if (gigaByte < 1) {
            ...
            return result2.setScale(newScale ?: 2, BigDecimal.ROUND_HALF_UP).toPlainString() + "MB"
        }
        ...
        if (teraBytes < 1) {
            ...
            return result3.setScale(newScale ?: 2, BigDecimal.ROUND_HALF_UP).toPlainString() + "GB"
        }

        val result4 = BigDecimal(teraBytes)
        return result4.setScale(newScale ?: 2, BigDecimal.ROUND_HALF_UP).toPlainString() + "TB"
    }
     ...
     * 格式化单位传入KB
     */
    fun getFormatSizeFromKB(size: Double): String {

        val megaByte = size / 1024
